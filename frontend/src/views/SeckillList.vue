            ...
            <div class="banner-right">
              <div class="session-info">
                <div class="session-tabs">
                  <button 
                    class="session-tab"
                    :class="{ active: activeSession === 'current' }"
                    @click="switchSession('current')"
                  >
                  ...
                  <button 
                    class="session-tab"
                    :class="{ active: activeSession === 'next' }"
                    @click="switchSession('next')"
                  >
          ...
          <div class="sidebar-section">
            ...
            <div class="filter-group">
              ...
              <div class="price-range">
                <input type="number" v-model.number="priceMin" placeholder="最低价" class="price-input" min="0" />
                <span class="price-sep">-</span>
                <input type="number" v-model.number="priceMax" placeholder="最高价" class="price-input" min="0" />
              </div>
            ...
            <div class="filter-group">
              ...
              <div class="discount-options">
                <button 
                  ...
                  @click="discountFilter = 'all'"
                >
                  全部
                </button>
                <button 
                  class="discount-btn"
                  :class="{ active: discountFilter === '3' }"
                  @click="discountFilter = '3'"
                >
                  3折以下
                </button>
                <button 
                  class="discount-btn"
                  :class="{ active: discountFilter === '5' }"
                  @click="discountFilter = '5'"
                >
                  5折以下
                </button>
                <button 
                  class="discount-btn"
                  :class="{ active: discountFilter === '7' }"
                  @click="discountFilter = '7'"
                >
                  7折以下
                </button>
              </div>
            ...
            <div class="filter-group">
              ...
              <div class="stock-options">
                ...
                <button 
                  class="stock-btn"
                  :class="{ active: stockFilter === 'available' }"
            ...
            >
              <div class="card-image">
                ...
                <div class="card-progress" v-if="product.status === 1 && product.seckillStock > 0">
                  <div class="progress-bar">
                    <div class="progress-fill" :style="{ width: `${getSoldPercent(product)}%` }"></div>
                  </div>
              ...
              <div class="card-body">
                ...
                <div class="card-price">
                  ...
                  <div class="price-original" v-if="product.originalPrice && product.originalPrice > product.seckillPrice">
...
const nextSessionTime = computed(() => {
  const now = new Date()
  const hour = now.getHours()
  if (hour < 10) return '12:00'
  if (hour < 12) return '14:00'
  if (hour < 14) return '16:00'
  if (hour < 16) return '18:00'
  if (hour < 18) return '20:00'
  if (hour < 20) return '22:00'
  if (hour < 22) return '00:00'
  return '10:00'
})
...
const nextSessionCount = computed(() => {
...
const filteredProducts = computed(() => {
  ...
  } else if (activeSession.value === 'next') {
    list = list.filter(p => p.status === 0)
  }
  ...
  if (priceMin.value !== null && priceMin.value >= 0) {
    list = list.filter(p => p.seckillPrice >= priceMin.value)
  }
  if (priceMax.value !== null && priceMax.value > 0) {
    list = list.filter(p => p.seckillPrice <= priceMax.value)
  }
  
  if (discountFilter.value !== 'all') {
    const maxDiscount = Number(discountFilter.value)
    list = list.filter(p => {
      if (!p.originalPrice) return false
      const discount = (p.seckillPrice / p.originalPrice) * 10
  ...
  if (stockFilter.value !== 'all') {
    if (stockFilter.value === 'available') {
      list = list.filter(p => p.seckillStock > 0)
    } else if (stockFilter.value === 'low') {
      list = list.filter(p => p.seckillStock > 0 && p.seckillStock <= 10)
    }
  }
  ...
  if (categoryQuery) {
    list = list.filter(p => p.categoryId === Number(categoryQuery))
  }
...
const sortedProducts = computed(() => {
  ...
  if (sortBy.value === 'price-asc') {
    ...
    list.sort((a, b) => b.seckillPrice - a.seckillPrice)
  } else if (sortBy.value === 'stock') {
    list.sort((a, b) => b.seckillStock - a.seckillStock)
  } else if (sortBy.value === 'discount') {
    list.sort((a, b) => {
      const discountA = a.originalPrice ? (a.seckillPrice / a.originalPrice) : 1
      const discountB = b.originalPrice ? (b.seckillPrice / b.originalPrice) : 1
...
function getDiscount(product) {
  ...
  const discount = Math.round((product.seckillPrice / product.originalPrice) * 10)
  return discount < 10 ? discount : 0
}

function getSoldPercent(product) {
  if (!product.totalStock || !product.seckillStock) return 50
  return Math.round(((product.totalStock - product.seckillStock) / product.totalStock) * 100)
}
...
function getProductCountdown(product) {
...
async function refreshStock() {
  for (const product of products.value) {
    if (product.status === 1 && product.seckillStock > 0) {
      try {
...
function updateSessionCountdown() {
  ...
  if (activeSession.value === 'current') {
    const activeProducts = products.value.filter(p => p.status === 1)
    if (activeProducts.length > 0) {
      targetTime = new Date(activeProducts[0].endTime)
    } else {
    ...
    const pendingProducts = products.value.filter(p => p.status === 0)
    if (pendingProducts.length > 0) {
      targetTime = new Date(pendingProducts[0].startTime)
    } else {
      const nextHour = new Date()
      nextHour.setHours(nextHour.getHours() + 4, 0, 0, 0)
      targetTime = nextHour
    }
  }
...
async function handleSeckill(product) {
  if (product.status === 0) {
    ElMessage.info('已设置秒杀提醒，开抢前会通知您')
    return
  }
  ...
  try {
    await ElMessageBox.confirm(
      `确定要抢购「${product.productName}」吗？每位用户仅有一次秒杀资格！`,
      '抢购确认',
      {
...
watch([statusFilter, selectedCategory, discountFilter, stockFilter, sortBy, activeSession], () => {
  currentPage.value = 1
})
...
.discount-options, .stock-options {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}

.discount-btn, .stock-btn {
  padding: 6px 12px;
  background: transparent;
  border: 1px solid #e5e5e5;
  border-radius: 6px;
...
.discount-btn.active, .stock-btn.active {
  background: #FF5000;
  border-color: #FF5000;
  color: #fff;
}
