  ...
  <div class="p-6">
    ...
    <n-modal v-model:show="showAdd" preset="card" title="添加敏感词" style="max-width: 400px">
      <n-form label-placement="left" label-width="80">
        ...
        <n-form-item v-if="form.level===2" label="替换内容"><n-input v-model:value="form.replace_to" placeholder="替换为" /></n-form-item>
      </n-form>
      <template #action>
...
const columns = [
  ...
  { title: '时间', key: 'created_at', width: 170, render: (row: any) => row.created_at ? new Date(row.created_at).toLocaleString('zh-CN') : '-' },
...
async function fetchItems() {
  loading.value = true
  try { const res = await get<any>('/admin/sensitive-words'); items.value = Array.isArray(res.data) ? res.data : [] } catch { items.value = [] } finally { loading.value = false }
}
