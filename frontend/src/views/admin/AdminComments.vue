...
const paginationReactive = reactive({ page: 1, pageSize: 20, itemCount: 0, onChange: (p: number) => { paginationReactive.page = p; fetchComments() } })
async function fetchComments() {
  loading.value = true
  try { const res = await get<any>('/admin/comments', { page: paginationReactive.page, page_size: 20 }); comments.value = res.data?.items || []; paginationReactive.itemCount = res.data?.total || 0 } catch { comments.value = [] } finally { loading.value = false }
}
function handleDelete(row: any) {
  dialog.warning({ title: '确认删除', content: '确定删除该评论？', positiveText: '删除', negativeText: '取消',
    onPositiveClick: async () => { try { await del('/admin/comments/' + row.id); comments.value = comments.value.filter((c: any) => c.id !== row.id); message.success('已删除') } catch { message.error('删除失败') } }
  })
}
